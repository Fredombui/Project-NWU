# -*- coding: utf-8 -*-
"""
Created on Mon Feb  5 23:58:49 2024
...
fr = pd.read_csv("movie_dataset.csv")

# Renaming each column with spaces inbetween

fr = fr.rename(columns = {'Runtime (Minutes)':'Runtime_(Minutes)'})
fr = fr.rename(columns = {'Revenue (Millions)':'Revenue_(Millions)'})


# This will drop all rows where the column'Revenue_(Millions)' has any null or empty values

new_fr = fr.dropna(subset=['Revenue_(Millions)'], how='all')

# This will drop all the specified columns (Relevance) 

new_fr = new_fr.drop(columns = "Rank")
new_fr = new_fr.drop(columns = "Description")
new_fr = new_fr.drop(columns = "Votes")
new_fr = new_fr.drop(columns = "Metascore")

###############################################################################
                             ####QUESTION 01####
###############################################################################

Highest_rated_movie = new_fr.loc[new_fr['Rating'].idxmax()]['Title']
...
Average_revenue = new_fr[Revenue].mean()
...
filtered_fr = new_fr[(new_fr[Year_column] >= 2015) & (new_fr[Year_column] <= 2017)]

# Using the mean() method on the filtered DataFrame 'fitered_fr' to calculate the average

New_Average_revenue = filtered_fr[Revenue].mean()
...
Movies_in_2016 = fr[fr[Year_column] == Target_year]

# Now 'Movies_in_2016' is a subset containing all rows where the release year is 2016

# Using the shape attribute to get the number of rows (movies) in the filtered DataFrame 'filtered_fr'
...
# Using boolean indexing to filter rows based on the specific director i.e 'Christopher Nolan' in the DataFrame 'fr'

Director = fr[fr[Director_names] == Target_director]
...
filtered_rating = fr[(fr[Rating_column] >= 8.0)]
...
Average_ratings_by_year = fr.groupby('Year')['Rating'].mean()
...
Movies_2006 = fr[fr['Year'] == 2006]
Movies_2016 = fr[fr['Year'] == 2016]
...
All_names = new_fr[Actors_column].str.split(',', expand=True).stack()
